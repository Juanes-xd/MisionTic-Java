...
public class ComponentePractico {
        ...
        while(opcionMenu >0 && opcionMenu <=6) {
            ...
            switch(opcionMenu) {
                ...
                case 7:
                    ...
                    System.out.println("Digite el tipo de producto, P para prendas, C para calzado");
                    String tipoComprar = scanner.nextLine();
                    
                    if (tipoComprar.equalsIgnoreCase("p")) {
                        int j =0;
                        boolean prendaEncontrada = false;
                        while(j < prendas.length && !prendaEncontrada) {
                            if(prendas[j].getCodigo().equals(codigoComprar)) {
                                prendaEncontrada = true;
                                // Validar si las unidades alcanzas
                                if (prendas[j].getCantidadBodega() + unidadesComprar <= prendas[j].getCantidadMaxima()) {
                                    double valorFactura = unidadesComprar * prendas[j].getPrecioVenta();
                                    double valorFacturaD = valorFactura * (1 - Producto.descuento);
                                    System.out.println("Valor factura: $" + valorFactura);
                                    System.out.println("Valor con descuento: $" + valorFacturaD);
                                    int nuevaCantidadBodega = prendas[j].getCantidadBodega() - unidadesComprar;
                                    prendas[j].setCantidadBodega(nuevaCantidadBodega);
                                    System.out.println("Compra exitosa!");
                                    System.out.println("Nueva cantidad en bodega: " + prendas[j].getCantidadBodega());
                                } else {
                                    System.out.println("No hay unidades suficientes para comprar de esta prenda.");
                                }
                            } else {
                                j += 1;
                            }
                    ...
                    } else if (tipoComprar.equalsIgnoreCase("c")) {
                        int k = 0;
                        boolean calzadoEncontrado = false;
                        while(k < calzados.length && !calzadoEncontrado) {
                            if(calzados[k].getCodigo().equals(codigoComprar)) {
                                calzadoEncontrado = true;
                                // Validar si hay unidades suficientes
                                if (calzados[k].getCantidadBodega() + unidadesComprar <= calzados[k].getCantidadMaxima()) {
                                    double valorFactura = unidadesComprar * calzados[k].getPrecioVenta();
                                    double valorFacturaD = valorFactura * (1 - Producto.descuento);
                                    System.out.println("Valor factura: $" + valorFactura);
                                    System.out.println("Valor con descuento: $" + valorFacturaD);
                                    int nuevaCantidadBodega = calzados[k].getCantidadBodega() - unidadesComprar;
                                    calzados[k].setCantidadBodega(nuevaCantidadBodega);
                                    System.out.println("Compra exitosa!");
                                    System.out.println("Nueva cantidad en bodega: " + calzados[k].getCantidadBodega());
                                } else {
                                    System.out.println("No hay unidades suficientes para comprar de este calzado.");
                                }
                            } else {
                                k += 1;
                            }
    ...
    //Cantidad maxima < cantidad en bodega + cantidad compra
    public static int mostrarMenu() {
        ...
        while (opcion < 0 || opcion > 7) {
            ...
            System.out.println("7. Comprar producto en el proveedor");
